...
const FeedItem = ({ imageUrl, nick, text, avatarUrl, time, typeInteraction, typeContent, socialNetwork, likes }) => {
  let hashtag = /#[a-zA-Z]+/.exec(text)[0]
  let [user, content] = text.split(hashtag)
      ...
      { (typeInteraction === 'creation') &&
        <div>
          <div className="feed-item-content">
            <span>{user}</span><span className="hashtag">{hashtag}</span><span>{content}</span>
          </div>
